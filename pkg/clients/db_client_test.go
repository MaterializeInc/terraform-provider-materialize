...
// Only the "+" that Encode writes for a space may become %20. A "+" that is
// part of the value itself is already %2B by then and has to stay a "+".
func TestConnectionStringKeepsLiteralPlus(t *testing.T) {
	r := require.New(t)
	...
	cfg, err := pgconn.ParseConfig(c)
	r.NoError(err)
	r.Equal(`--transaction_isolation=strict\ serializable --search_path=a+b\ c`, cfg.RuntimeParams["options"])
}
