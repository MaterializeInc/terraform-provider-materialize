...
func TestResourceGrantSourceDefaultPrivilegeCreate(t *testing.T) {
	r := require.New(t)
	...
	d := schema.TestResourceDataRaw(t, GrantSourceDefaultPrivilege().Schema, in)
	r.NotNil(d)
	...
	testhelpers.WithMockDb(t, func(db *sqlx.DB, mock sqlmock.Sqlmock) {
		...
		qp := `
			...
			AND mz_default_privileges.object_type = 'source'
			AND mz_default_privileges.role_id = 'u1'`
		testhelpers.MockDefaultPrivilegeScan(mock, qp, "source")

		if err := grantSourceDefaultPrivilegeCreate(context.TODO(), d, db); err != nil {
			t.Fatal(err)
		}

		if d.Id() != "GRANT DEFAULT|SOURCE|u1|u1|||USAGE" {
			t.Fatalf("unexpected id of %s", d.Id())
		}
	})
}

func TestResourceGrantSourceDefaultPrivilegeDelete(t *testing.T) {
	r := require.New(t)
	...
	testhelpers.WithMockDb(t, func(db *sqlx.DB, mock sqlmock.Sqlmock) {
		...
		if err := grantSourceDefaultPrivilegeDelete(context.TODO(), d, db); err != nil {
			t.Fatal(err)
		}
	})
}
