...
func TestResourceSourceTableWebhookCreateWithTextFormat(t *testing.T) {
	r := require.New(t)
	...
	d := schema.TestResourceDataRaw(t, SourceTableWebhook().Schema, in)
	r.NotNil(d)

	testhelpers.WithMockProviderMeta(t, func(db *utils.ProviderMeta, mock sqlmock.Sqlmock) {
		// Create
		mock.ExpectExec(
			`CREATE TABLE "database"."schema"."webhook_table" FROM WEBHOOK BODY FORMAT TEXT;`,
		).WillReturnResult(sqlmock.NewResult(1, 1))
...
func TestResourceSourceTableWebhookCreateWithIncludeHeader(t *testing.T) {
	r := require.New(t)
	in := map[string]interface{}{
		"name":          "webhook_table",
		"schema_name":   "schema",
	...
	testhelpers.WithMockProviderMeta(t, func(db *utils.ProviderMeta, mock sqlmock.Sqlmock) {
		// Create
		mock.ExpectExec(
			`CREATE TABLE "database"."schema"."webhook_table" FROM WEBHOOK BODY FORMAT JSON INCLUDE HEADER 'x-api-key' AS api_key;`,
		).WillReturnResult(sqlmock.NewResult(1, 1))
