...
var inSourceSQLServer = map[string]interface{}{
	...
	"text_columns":    []interface{}{"xml_column", "ntext_column"},
	"exclude_columns": []interface{}{"geometry_column", "geography_column"},
	"table": []interface{}{
		map[string]interface{}{
...
func TestResourceSourceSQLServerCreate(t *testing.T) {
	...
	testhelpers.WithMockProviderMeta(t, func(db *utils.ProviderMeta, mock sqlmock.Sqlmock) {
		// Create
		mock.ExpectExec(
			`CREATE SOURCE "database"."schema"."source" IN CLUSTER "test_cluster" FROM SQL SERVER CONNECTION "database"."schema"."sqlserver_connection" \(TEXT COLUMNS \(xml_column, ntext_column\), EXCLUDE COLUMNS \(geometry_column, geography_column\)\) FOR TABLES \("dbo"."table1" AS "database"."schema"."renamed_table1", "custom"."table2" AS "database"."schema"."table2"\) EXPOSE PROGRESS AS "database"."schema"."progress";`,
...
func TestResourceSourceSQLServerUpdate(t *testing.T) {
	...
	testhelpers.WithMockProviderMeta(t, func(db *utils.ProviderMeta, mock sqlmock.Sqlmock) {
		...
		mock.ExpectExec(`ALTER SOURCE "database"."schema"."source" ADD SUBSOURCE "dbo"."table1" AS "database"."schema"."renamed_table1", "custom"."table2" WITH \(TEXT COLUMNS \[xml_column, ntext_column\]\);`).WillReturnResult(sqlmock.NewResult(1, 1))
