		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckGrantMaterializedViewExists("materialize_grant_materialized_view.materialized_view_grant", roleName, materializedViewName, schemaName, databaseName, privilege),
					resource.TestCheckResourceAttr("materialize_grant_materialized_view.materialized_view_grant", "role_name", roleName),
					resource.TestCheckResourceAttr("materialize_grant_materialized_view.materialized_view_grant", "privilege", privilege),
					resource.TestCheckResourceAttr("materialize_grant_materialized_view.materialized_view_grant", "materialized_view_name", materializedViewName),
					resource.TestCheckResourceAttr("materialize_grant_materialized_view.materialized_view_grant", "schema_name", schemaName),
					resource.TestCheckResourceAttr("materialize_grant_materialized_view.materialized_view_grant", "database_name", databaseName),
				),
			},
		},
	})
...
resource "materialize_grant_materialized_view" "materialized_view_grant" {
...
func testAccCheckGrantMaterializedViewExists(grantName, roleName, materializedViewName, schemaName, databaseName, privilege string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		id, err := materialize.MaterializedViewId(db, materializedViewName, schemaName, databaseName)
		if err != nil {
			return err
		}
