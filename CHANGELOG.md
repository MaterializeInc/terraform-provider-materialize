...
* `materialize_connection_iceberg_catalog` now supports `catalog_type = "rest"` for any Iceberg REST catalog, including Databricks Unity Catalog, with `credential` (a secret or text holding `<client_id>:<client_secret>`), `oauth2_server_url`, `scope` and `access_delegation`. `warehouse` and `aws_connection` are now optional since a REST catalog does not need them.
* `materialize_sink_iceberg` gained `mode`, which defaults to `upsert` and accepts `append` for tables that only take appends, such as Databricks Unity Catalog. `key` is now optional: the plan requires it for `upsert` and rejects it for `append`. `aws_connection` is optional too, as the sink inherits storage credentials from the catalog connection. The mode is read back from the catalog so imports pick it up.
