...
#### Option 1: Recreate Using the New Resource

If you can tolerate downtime, recreate the webhook using the new `materialize_table_from_webhook` resource:
...
#### Option 2: Continue Using the Legacy Resource

If you cannot tolerate downtime or need to preserve existing data, continue using `materialize_source_webhook` until an automated migration path becomes available on the database side. The legacy resource remains fully supported.
